...
function passwordResetOtpTarget(email: string) {
  // Keep password-reset OTPs isolated from sign-up/email-verification OTPs,
  // even if the OTP storage backend keys by target.
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const code = await createOtp(passwordResetOtpTarget(email), 'PASSWORD_RESET');
    await sendOtpEmail(email, code);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const email = req.body.email.toLowerCase().trim();
    const valid = await verifyOtp(passwordResetOtpTarget(email), 'PASSWORD_RESET', req.body.code);
