...
function ellipsize(value: unknown, maxChars: number): string {
  ...
  return `${v.slice(0, Math.max(0, maxChars - 3))}...`;
}
...
function drawSectionTitle(title: string, x: number, y: number): PdfCommand {
  // Clean compact section title: no underline, so the title never collides with the next table.
  return [
    rect(x, y - 12, 4, 14, GOLD, GOLD),
    text(title, x + 10, y - 8, 11.2, 'F2', DARK),
  ].join('');
}
...
function drawSummaryRow(
  ...
  const valueLines = wrap(safe(value), Math.max(10, Math.floor((valueW - 18) / 4.4))).slice(0, maxValueLines);
  const baseY = yTop - rowH;

  return [
    rect(x, baseY, labelW, rowH, '0.88 0.88 0.88', '0.985 0.985 0.985'),
    rect(x + labelW, baseY, valueW, rowH, '0.88 0.88 0.88', '1 1 1'),
    text(label, x + 9, yTop - 15, 7.6, 'F2', DARK),
    ...valueLines.map((lineText, index) =>
      text(lineText, x + labelW + 9, yTop - 15 - index * 9, 7.5, 'F1', DARK),
    ),
  ].join('');
}
...
function buildReceiptPdf(order: AnyOrder): Buffer {
  ...
  // Summary panel - one aligned grid instead of floating mini boxes.
  commands.push(drawSectionTitle('Receipt Summary', pageLeft, y));
  y -= 18;
  const summaryX = pageLeft;
  const summaryTop = y;
  const summaryH = 84;
  const summaryGap = 12;
  const summaryColW = (tableWidth - summaryGap) / 2;
  const labelW = 104;
  const valueW = summaryColW - labelW;
  const rowH = summaryH / 3;
  commands.push(rect(summaryX, summaryTop - summaryH, tableWidth, summaryH, '0.82 0.82 0.82', LIGHT_FILL));
  commands.push(drawSummaryRow('Amount paid', money(order.total), summaryX, summaryTop, labelW, valueW, rowH));
  commands.push(drawSummaryRow('Payment method', paymentMethod(order), summaryX, summaryTop - rowH, labelW, valueW, rowH));
  commands.push(drawSummaryRow('Payment status', paymentStatus(order), summaryX, summaryTop - rowH * 2, labelW, valueW, rowH));

  const rightX = summaryX + summaryColW + summaryGap;
  commands.push(drawSummaryRow('Payment reference', paymentReference(order), rightX, summaryTop, labelW, valueW, rowH * 2, 3));
  commands.push(drawSummaryRow('Delivered on', markedDeliveredAt, rightX, summaryTop - rowH * 2, labelW, valueW, rowH));
  y -= summaryH + 22;

  // Customer and delivery details.
  commands.push(drawSectionTitle('Customer & Delivery Details', pageLeft, y));
  y -= 18;
  commands.push(rect(pageLeft, y - 92, tableWidth, 92, '0.82 0.82 0.82', LIGHT_FILL));
  commands.push(fieldBox('Customer name', customerName(order), pageLeft + 12, y - 38, 152, 32));
  commands.push(fieldBox('Phone', order.shippingAddress?.phone || order.user?.phone, pageLeft + 174, y - 38, 130, 32));
  commands.push(fieldBox('Email', customerEmail(order) || order.shippingAddress?.email, pageLeft + 12, y - 76, 292, 32));
  commands.push(text('Delivery address', pageLeft + 324, y - 19, 8, 'F2', MUTED));
  wrap(address, 40).slice(0, 5).forEach((wrappedLine, index) => {
    commands.push(text(wrappedLine, pageLeft + 324, y - 34 - index * 10, 8, 'F1', DARK));
  });
  y -= 112;

  // Items table. The columns are wider and all text is wrapped or clipped before drawing.
  // Extra spacing below the title prevents any subtitle/header collision.
